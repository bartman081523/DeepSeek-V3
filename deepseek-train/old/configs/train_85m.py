...
if __name__ == '__main__':
    ...
    if init_from == 'scratch':
        model_args = dict(
            ...
            n_dense_layers=4,    # Keep equal to n_layers, since no MoE
            ...
            qk_nope_head_dim=32,  # Reduced head dimension
            qk_rope_head_dim=32, # Reduced head dimension
            v_head_dim=32,        # Reduced head dimension
            original_seq_len=4096,
            rope_theta=10000.0,
            rope_factor=1.0,    # Disable scaling
            beta_fast=32,
            beta_slow=1,
            mscale=1.0,
            dim=384,            # Reduced dimension.  This is a key driver.
            inter_dim=1536,   # Keep this as 4 * dim
            moe_inter_dim=1536,  # Match inter_dim, though it's not used
            max_batch_size=batch_size,
            max_seq_len=block_size,
            vocab_size=vocab_size,  # Keep fixed
            dtype=dtype,
        )

        model_args = ModelArgs(**model_args)
        model = Transformer(model_args)
        print(f"Parameter count: {model.get_parameter_count():,}")  # Check the parameter count
