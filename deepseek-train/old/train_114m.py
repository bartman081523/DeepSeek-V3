...
if __name__ == '__main__':
    ...
    if init_from == 'scratch':
        model_args = dict(
            ...
            n_dense_layers=6,   # Set to n_layers if no MoE
            ...
            q_lora_rank = 0,
            kv_lora_rank = 0,
            qk_nope_head_dim = 32,  # Reduce head dimensions
            qk_rope_head_dim = 32, # Reduced.
            v_head_dim= 32,         # Reduced.
            original_seq_len = 4096,
            rope_theta = 10000.0,
            rope_factor=1.0,        # Disable
            beta_fast = 32,
            beta_slow=1,
            mscale = 1.0,
            dim=512,              # Significantly reduced dimension
            inter_dim=1024,     # Adjust based on dim.
            moe_inter_dim=1024,
            max_batch_size=batch_size,
            max_seq_len=block_size,
            vocab_size=vocab_size,    # Keep this fixed!
            dtype=dtype,
        )

        model_args = ModelArgs(**model_args)
        model = Transformer(model_args)
        print(f"Parameter count: {model.get_parameter_count():,}")  # Print parameter count
